...
"""
Set of functions to save and load from XML file/Strings any Parameter object
"""
import importlib
import json
...
def add_text_to_elt(elt, param):
    """Add text fields in a xml element corresponding to the parameter
    ...
    if 'bool' in param_type or 'led' in param_type:
        ...
        text = str(QtCore.QDateTime(param.value()).toSecsSinceEpoch())
...
def parameter_to_xml_file(param, filename):
    """
        Convert the given parameter to XML element and create the given XML file.
...
def set_txt_from_elt(el, param_dict):
    ...
    if val_text is not None:
        if param_type == 'float':
            ...
            param_value = QtCore.QDateTime.fromSecsSinceEpoch(int(val_text))
        elif param_type == 'date':
            param_value = QtCore.QDateTime.fromSecsSinceEpoch(int(val_text)).date()
